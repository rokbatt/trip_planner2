...
--
-- 무엇을 캐싱하나
--   kind='route-plan'  : 여행지 전체 DAY 배분 추천 (AI 일정 짜기 버튼)
...
--   달라지는 값"을 전부 녹여서, Brainstorm 장소 목록이나 숙소가 그대로면 몇 번을 눌러도
--   Gemini 호출은 0회가 되게 한다.
--     route-plan 키 = 여행지 + DAY 수 + 숙소 + 장소 id 목록(정렬) 의 해시
--     day-detail 키 = 여행지 + DAY 번호 + 그 DAY 정류지 순서 의 해시
--   → 장소를 담거나 빼거나 숙소를 바꾸면 키가 자동으로 달라져 새로 생성됨.
--
...
create table if not exists ai_plan_cache (
  ...
  kind       text not null,      -- 'route-plan' | 'day-detail'
