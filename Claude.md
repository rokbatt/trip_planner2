...
- **일정 카드**(좌): "Airport Lounge Boarding-Pass" 레퍼런스를 그대로 따른 구조 — 순번(Navy 원형
  32px) → 사진(1.8:1 썸네일, 카드 높이와 무관하게 고정 비율) → 이름+별점+카테고리+영업시간 배지
  → 메모(항상 또렷하게 보이는 입력칸, 비어 있다고 흐리게 숨기지 않음) → 우측 체류 요약 박스
  (예상 체류/도착/출발, 폭 130px, 배경 `#FCFDFF`). **구글 평점·카테고리·영업시간은 실제 저장된
  값이 있을 때만** 보여준다 — 입장료·체크인 시각·WiFi처럼 우리 DB에 없는 값은 지어내지 않는다
  (원칙 3-1). 영업시간은 `place.opening_hours`(Google weekdayDescriptions, 월요일부터 7줄)에서
  그 정류지가 속한 캘린더 날짜의 요일에 맞는 줄만 골라 보여준다(`todaysHoursLine`).
...
- **이동 구간은 Divider가 아니라 연결 카드**(`.tl-legcard`, 높이 약 52~56px) — 이동수단별 옅은
  Tint 원형 아이콘(택시 `rgba(246,181,69,.14)`/도보 `rgba(83,185,156,.14)`) + 소요시간·거리·요금 +
  우측 "N시 도착 예정 ▼". 이 화살표를 눌러야만 이동수단 선택 버튼이 펼쳐진다(카드 전체가 아니라
  이 버튼만 상호작용 지점).
...
- **모바일(≤720px)**: 사진·장소 메모를 접어 카드를 두어 줄로 줄인다. 지도 토글 자체는 모바일에서도
  그대로 살아 있다("항상 열고닫기가 되어야 한다" — 화면 폭에 따라 기능이 사라지면 안 됨)
