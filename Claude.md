...
- **일정 카드**(좌): 사진·평점·주소·Brainstorm 메모(불릿)·이 날의 메모가 카드 한 장에 다 보인다.
  접었다 폈다 하지 않는 이유는 이 화면이 "훑어보는 목록"이 아니라 여행 계획서 그 자체라,
  무엇이 담겨 있는지 클릭해봐야 아는 상태를 만들지 않기 위해서다.
...
- **전체 보기**: 모든 DAY를 컬럼으로 나란히. 컬럼마다 같은 24시간 축의 띠를 붙여 날짜 간 비교가
  목적인 뷰라, 편집은 막고 읽기에만 집중(수정은 하루 보기에서). 이 뷰에선 지도를 접는다
- **여행 중**: 오늘이 여행 기간 안이면 그 DAY로 바로 열리고, 일정 사이에 현재 시각선이 뜬다
- **모바일(≤720px)**: 사진·주소·장소 메모를 접어 카드를 두어 줄로 줄인다. 지도는 ≤1080px에서
  접히고, 함께 "지도에서 보기" 버튼도 숨긴다(갈 곳이 없는 버튼을 남기지 않기 위해)
