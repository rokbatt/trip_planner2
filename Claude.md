...
DAY를 바꾸거나 "지도에서 보기"를 누르면 상세 패널은 닫히고 실제 지도로 돌아간다.

- **머리**: 이름 · 카테고리 · 평점 · 사진(16:9) · 닫기(✕). **즐겨찾기·"더보기" 메뉴는 아직 없다** —
  앱 전체에 즐겨찾기 기능 자체가 없어서 지금 넣으면 눌러도 아무 일도 안 일어나는 반쪽 기능이 된다.
  실제 즐겨찾기 저장 구조가 생기면 그때 추가할 것.
- **탭**: `개요` / `지도` / `리뷰`. **`지도` 탭은 상태가 아니라 동작**이다 — 눌러도 탭 내용이
  바뀌는 게 아니라 패널이 닫히고 실제 지도가 그 정류지로 이동한다(장소별 미니 지도를 따로
  만들지 않고, 이미 있는 하루 지도를 재사용).
- **개요 탭 = Place Brief + Before You Go + Don't Miss.** 이 세 섹션 대부분은 원래 AI 요약·추천이
  들어갈 자리지만, **AI 연동은 다음 단계**이고 원칙 3-1(데이터를 지어내지 않는다)이 있어 지금은
  실제로 있는 값만 채우고 나머지는 정직한 스텁으로 남겨둔다(워크스페이스 보드 상세 Drawer의
  "AI 요약은 다음 단계에서 연결돼요" 문구와 같은 패턴):
  - Place Brief: AI 설명 문구는 스텁. 대신 실제로 만들 수 있는 것만 — Google 지도 링크(place_id
    기반), Wikipedia 검색 링크(둘 다 API 호출 없이 URL만 조합, 원칙 3-2와 무관), 이미 앱 전체가
    쓰는 체류시간 추정치(`dwellMinutes`)와 카테고리 칩.
  - Before You Go: `운영시간` 타일만 실제 데이터(`place.opening_hours`, 오늘 요일 굵게).
    복장 규정·준비물·방문 팁 등은 DB에 없는 값이라 지어내지 않고 점선 테두리 스텁 타일 하나로
    묶었다. 옆의 **개인 메모**는 AI가 아니라 사용자 입력 — 노란 포스트잇이 아니라 연한 Warm
    Gray(`#FAF9F7`) 카드이고, 실제로는 카드 목록의 한 줄 메모(`route_stops.memo`)와 **같은
    값**이다(장소 단위가 아니라 이 정류지 단위 메모 — 같은 장소를 여러 날 방문하면 메모는
    날마다 따로다). 입력 중 전체를 다시 그리면 포커스가 날아가므로 값만 갱신하고 카드 쪽
    입력창도 DOM에서 직접 맞춰준다.
  - Don't Miss: 추천 데이터가 전혀 없어 캐러셀 트랙에 스텁 카드 하나만 둔다. 나중에 추천이
    생기면 이 트랙에 카드를 늘어놓기만 하면 된다(구조는 이미 캐러셀 모양).
  - 리뷰 탭: 리뷰 텍스트를 모아오는 기능이 없어 평균 평점만 보여주고 나머지는 Google 지도·
    Google 검색 링크로 보낸다.
- **사진은 지금 장소당 1장뿐**(`places.photo_url`, 단일 필드)이라 캐러셀·"1/12" 인디케이터
  없이 큰 사진 1장만 보여준다. Google Places는 장소당 여러 사진을 주지만 지금은 `photos[0]`만
  받아서 저장한다 — 여러 장을 받게 되면 이 사진 영역에 캐러셀을 그대로 얹을 수 있게
  `.tl-pd-photo`를 이미 그 구조로 비워 뒀다.
- 색은 새 팔레트를 만들지 않고 기존 Airport Lounge 토큰(`--tl-c-primary`, `--tl-blue`,
  `--tl-c-card-border` 등)을 그대로 썼다.
