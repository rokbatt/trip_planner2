...
- **AI가 아직 없는 자리에 지어낸 글을 채우지 않는다(원칙 3-1).** 대신 "개발 중" 문구를 본문에
  노출하지 않고 **생성 버튼 하나만** 둔다(`AI 요약 생성`, `추천 포인트 생성`). 눌러도 아무 반응이
  없으면 고장으로 보이므로 클릭 시 상태 문구만 한 줄 뜬다(`bindGenButton`). AI 단계가 붙으면
  이 버튼의 핸들러만 실제 호출로 바꾸면 된다.
