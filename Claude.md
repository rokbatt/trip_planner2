...
**컬러 컨셉은 "Night Lounge"** — 데스크톱이 낮의 밝은 라운지라면 모바일은 같은 라운지의 밤이다.
고급스러움은 색을 많이 쓰는 데서 오지 않고 **색 온도 대비**에서 온다: 차갑고 깊은 바탕 +
따뜻한 황동 조명. 실제 라운지의 재료가 그렇다.

| 역할 | 값 | 쓰는 곳 |
|---|---|---|
| 바탕 | `#101925`→`#0B121C`→`#070C13` | 채도 낮은 슬레이트 잉크. **채도 높은 네이비를 전면에 깔지 않는다** |
| 조명 | 황동빛 radial + 옅은 하늘빛 radial | 위쪽에 광원 두 개를 겹침 — 단색 그라데이션 하나면 평평해 보인다 |
| 황동 | `#C7AA74` / `#E2C795` | 구조·장식: 시간축, 섹션 아이콘, 헤어라인, 활성 탭, 별점, AI |
| 하늘 | `#7FC2E4` | 브랜드 연결. **우리 DB의 실제 값**과 링크에만 |
| 주황 | `#F4801F` | 지금 진행 중인 일정 딱 하나 |
| 이동수단 | 세이지·스틸블루·테라코타 | 채도를 낮춘 자연색 (원색 3개를 나란히 두면 유아용처럼 보인다) |

**색에도 의미를 고정한다**(원칙 3-1의 "출처를 섞지 않는다"를 색으로도 지킨다) —
황동 = 구조·AI가 만든 값 / 하늘 = 우리 DB의 실제 값 / 회색 = 추정 / 주황 = 지금.

> ⚠️ **사진 위 스크림은 남색이 아니라 중성 먹색**(`--mb-scrim: 13,12,11`)이다. 남색 스크림을
> 씌우면 사원의 금빛도 바다의 청록도 전부 같은 파랑으로 죽어 화면이 단색이 된다. 사진이 자기
> 색을 유지해야 콘텐츠가 알아서 색 다양성을 만든다 — 실제로 첫 버전이 이 문제로 되돌려졌다.
> 헤어라인을 파란색이 아니라 황동빛으로 두는 것도 같은 이유다(한 가지 파랑의 투명도 변주로
> 테두리·배경·아이콘을 전부 처리하면 단색이 된다).
