...
- **일정 카드**(좌): **이름과 메모에만 집중한다.** 구글 평점·주소·카테고리 아이콘 같은 메타데이터는
  일부러 넣지 않았다 — 이 카드에서 실제로 보고 싶은 건 "몇 시에 어디를, 뭐라고 메모해 뒀는지"뿐이다.
  사진과 Brainstorm 메모(불릿)는 유지(사용자가 직접 남긴 정보라 근거 데이터와 다르게 취급).
  `시각 / 스파인 / 카드` 3열 격자는 모든 줄이 공유하고, 도착 시각과 점은 **카드 제목 줄에 맞춘다**
  (세로 가운데에 두면 사진·메모가 많은 카드에서 시각이 제목과 한참 떨어져 보인다)
- **핀·점 색은 카테고리 무지개색이 아니라 스카이블루 계열로 통일**(`pinColorFor` in timeline.ts) —
  아이콘을 없앤 이상 카테고리를 색으로 구분할 이유가 없다. 숙소·공항(하루의 시작·끝점)만 짙은
  남색(`#0B2A5C`)으로, 나머지는 전부 같은 블루(`#0B7CC4`). 이동수단 색(도보=초록/대중교통=파랑/
  택시=주황)은 "장소"가 아니라 "이동"의 속성이라 그대로 둔다.
...
- **이동수단**은 카드가 아니라 **구간(leg)을 눌러** 바꾼다 — "두 일정 사이"의 속성이라 그 자리가 맞다
